...
scrape_configs:
  ...
  - job_name: 'push'
    scrape_interval: 5s
    static_configs:
      - targets: [{% for instance in push_instances %}'{{instance}}'{{ ", " if not loop.last else "" }}{% endfor %}]
        labels:
          group: 'production'
